from http import HTTPStatus
from typing import Any

import httpx

from ... import errors
from ...client import AuthenticatedClient, Client
from ...models.http_validation_error import HTTPValidationError
from ...models.integration_category import IntegrationCategory
from ...models.marketplace_app import MarketplaceApp
from ...types import UNSET, Response, Unset


def _get_kwargs(
    *,
    category: IntegrationCategory | None | Unset = UNSET,
    search: None | str | Unset = UNSET,
    org_id: None | str | Unset = UNSET,
    x_org_id: None | str | Unset = UNSET,
) -> dict[str, Any]:
    headers: dict[str, Any] = {}
    if not isinstance(x_org_id, Unset):
        headers["X-Org-ID"] = x_org_id

    params: dict[str, Any] = {}

    json_category: None | str | Unset
    if isinstance(category, Unset):
        json_category = UNSET
    elif isinstance(category, IntegrationCategory):
        json_category = category.value
    else:
        json_category = category
    params["category"] = json_category

    json_search: None | str | Unset
    if isinstance(search, Unset):
        json_search = UNSET
    else:
        json_search = search
    params["search"] = json_search

    json_org_id: None | str | Unset
    if isinstance(org_id, Unset):
        json_org_id = UNSET
    else:
        json_org_id = org_id
    params["org_id"] = json_org_id

    params = {k: v for k, v in params.items() if v is not UNSET and v is not None}

    _kwargs: dict[str, Any] = {
        "method": "get",
        "url": "/api/v1/integrations/apps",
        "params": params,
    }

    _kwargs["headers"] = headers
    return _kwargs


def _parse_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
) -> HTTPValidationError | list[MarketplaceApp] | None:
    if response.status_code == 200:
        response_200 = []
        _response_200 = response.json()
        for response_200_item_data in _response_200:
            response_200_item = MarketplaceApp.from_dict(response_200_item_data)

            response_200.append(response_200_item)

        return response_200

    if response.status_code == 422:
        response_422 = HTTPValidationError.from_dict(response.json())

        return response_422

    if client.raise_on_unexpected_status:
        raise errors.UnexpectedStatus(response.status_code, response.content)
    else:
        return None


def _build_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
) -> Response[HTTPValidationError | list[MarketplaceApp]]:
    return Response(
        status_code=HTTPStatus(response.status_code),
        content=response.content,
        headers=response.headers,
        parsed=_parse_response(client=client, response=response),
    )


def sync_detailed(
    *,
    client: AuthenticatedClient,
    category: IntegrationCategory | None | Unset = UNSET,
    search: None | str | Unset = UNSET,
    org_id: None | str | Unset = UNSET,
    x_org_id: None | str | Unset = UNSET,
) -> Response[HTTPValidationError | list[MarketplaceApp]]:
    """List available integrations

     Browse the integration marketplace catalog.

    Returns all public integrations plus any private apps registered by this org.
    Optionally filter by category or text search.

    Args:
        category (IntegrationCategory | None | Unset): Filter by category
        search (None | str | Unset): Text search on name and description
        org_id (None | str | Unset): Organization ID (query parameter, overrides header)
        x_org_id (None | str | Unset): Organization ID (header)

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        Response[HTTPValidationError | list[MarketplaceApp]]
    """

    kwargs = _get_kwargs(
        category=category,
        search=search,
        org_id=org_id,
        x_org_id=x_org_id,
    )

    response = client.get_httpx_client().request(
        **kwargs,
    )

    return _build_response(client=client, response=response)


def sync(
    *,
    client: AuthenticatedClient,
    category: IntegrationCategory | None | Unset = UNSET,
    search: None | str | Unset = UNSET,
    org_id: None | str | Unset = UNSET,
    x_org_id: None | str | Unset = UNSET,
) -> HTTPValidationError | list[MarketplaceApp] | None:
    """List available integrations

     Browse the integration marketplace catalog.

    Returns all public integrations plus any private apps registered by this org.
    Optionally filter by category or text search.

    Args:
        category (IntegrationCategory | None | Unset): Filter by category
        search (None | str | Unset): Text search on name and description
        org_id (None | str | Unset): Organization ID (query parameter, overrides header)
        x_org_id (None | str | Unset): Organization ID (header)

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        HTTPValidationError | list[MarketplaceApp]
    """

    return sync_detailed(
        client=client,
        category=category,
        search=search,
        org_id=org_id,
        x_org_id=x_org_id,
    ).parsed


async def asyncio_detailed(
    *,
    client: AuthenticatedClient,
    category: IntegrationCategory | None | Unset = UNSET,
    search: None | str | Unset = UNSET,
    org_id: None | str | Unset = UNSET,
    x_org_id: None | str | Unset = UNSET,
) -> Response[HTTPValidationError | list[MarketplaceApp]]:
    """List available integrations

     Browse the integration marketplace catalog.

    Returns all public integrations plus any private apps registered by this org.
    Optionally filter by category or text search.

    Args:
        category (IntegrationCategory | None | Unset): Filter by category
        search (None | str | Unset): Text search on name and description
        org_id (None | str | Unset): Organization ID (query parameter, overrides header)
        x_org_id (None | str | Unset): Organization ID (header)

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        Response[HTTPValidationError | list[MarketplaceApp]]
    """

    kwargs = _get_kwargs(
        category=category,
        search=search,
        org_id=org_id,
        x_org_id=x_org_id,
    )

    response = await client.get_async_httpx_client().request(**kwargs)

    return _build_response(client=client, response=response)


async def asyncio(
    *,
    client: AuthenticatedClient,
    category: IntegrationCategory | None | Unset = UNSET,
    search: None | str | Unset = UNSET,
    org_id: None | str | Unset = UNSET,
    x_org_id: None | str | Unset = UNSET,
) -> HTTPValidationError | list[MarketplaceApp] | None:
    """List available integrations

     Browse the integration marketplace catalog.

    Returns all public integrations plus any private apps registered by this org.
    Optionally filter by category or text search.

    Args:
        category (IntegrationCategory | None | Unset): Filter by category
        search (None | str | Unset): Text search on name and description
        org_id (None | str | Unset): Organization ID (query parameter, overrides header)
        x_org_id (None | str | Unset): Organization ID (header)

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        HTTPValidationError | list[MarketplaceApp]
    """

    return (
        await asyncio_detailed(
            client=client,
            category=category,
            search=search,
            org_id=org_id,
            x_org_id=x_org_id,
        )
    ).parsed
