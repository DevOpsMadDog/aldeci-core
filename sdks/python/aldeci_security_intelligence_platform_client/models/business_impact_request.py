from __future__ import annotations

from collections.abc import Mapping
from typing import Any, TypeVar, cast

from attrs import define as _attrs_define
from attrs import field as _attrs_field

from ..types import UNSET, Unset

T = TypeVar("T", bound="BusinessImpactRequest")


@_attrs_define
class BusinessImpactRequest:
    """
    Attributes:
        target (None | str | Unset): Target service name
        cve_ids (list[str] | None | Unset): List of CVE IDs
    """

    target: None | str | Unset = UNSET
    cve_ids: list[str] | None | Unset = UNSET
    additional_properties: dict[str, Any] = _attrs_field(init=False, factory=dict)

    def to_dict(self) -> dict[str, Any]:
        target: None | str | Unset
        if isinstance(self.target, Unset):
            target = UNSET
        else:
            target = self.target

        cve_ids: list[str] | None | Unset
        if isinstance(self.cve_ids, Unset):
            cve_ids = UNSET
        elif isinstance(self.cve_ids, list):
            cve_ids = self.cve_ids

        else:
            cve_ids = self.cve_ids

        field_dict: dict[str, Any] = {}
        field_dict.update(self.additional_properties)
        field_dict.update({})
        if target is not UNSET:
            field_dict["target"] = target
        if cve_ids is not UNSET:
            field_dict["cve_ids"] = cve_ids

        return field_dict

    @classmethod
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        d = dict(src_dict)

        def _parse_target(data: object) -> None | str | Unset:
            if data is None:
                return data
            if isinstance(data, Unset):
                return data
            return cast(None | str | Unset, data)

        target = _parse_target(d.pop("target", UNSET))

        def _parse_cve_ids(data: object) -> list[str] | None | Unset:
            if data is None:
                return data
            if isinstance(data, Unset):
                return data
            try:
                if not isinstance(data, list):
                    raise TypeError()
                cve_ids_type_0 = cast(list[str], data)

                return cve_ids_type_0
            except (TypeError, ValueError, AttributeError, KeyError):
                pass
            return cast(list[str] | None | Unset, data)

        cve_ids = _parse_cve_ids(d.pop("cve_ids", UNSET))

        business_impact_request = cls(
            target=target,
            cve_ids=cve_ids,
        )

        business_impact_request.additional_properties = d
        return business_impact_request

    @property
    def additional_keys(self) -> list[str]:
        return list(self.additional_properties.keys())

    def __getitem__(self, key: str) -> Any:
        return self.additional_properties[key]

    def __setitem__(self, key: str, value: Any) -> None:
        self.additional_properties[key] = value

    def __delitem__(self, key: str) -> None:
        del self.additional_properties[key]

    def __contains__(self, key: str) -> bool:
        return key in self.additional_properties
