from __future__ import annotations

from collections.abc import Mapping
from typing import Any, TypeVar, cast

from attrs import define as _attrs_define
from attrs import field as _attrs_field

from ..types import UNSET, Unset

T = TypeVar("T", bound="SecretsScanContentRequest")


@_attrs_define
class SecretsScanContentRequest:
    """Request model for scanning content for secrets.

    Attributes:
        content (str): File content to scan
        filename (str): Filename
        repository (str | Unset): Repository name Default: 'inline'.
        branch (str | Unset): Branch name Default: 'main'.
        scanner (None | str | Unset): Scanner to use: 'gitleaks' or 'trufflehog' (auto-selected if not specified)
    """

    content: str
    filename: str
    repository: str | Unset = "inline"
    branch: str | Unset = "main"
    scanner: None | str | Unset = UNSET
    additional_properties: dict[str, Any] = _attrs_field(init=False, factory=dict)

    def to_dict(self) -> dict[str, Any]:
        content = self.content

        filename = self.filename

        repository = self.repository

        branch = self.branch

        scanner: None | str | Unset
        if isinstance(self.scanner, Unset):
            scanner = UNSET
        else:
            scanner = self.scanner

        field_dict: dict[str, Any] = {}
        field_dict.update(self.additional_properties)
        field_dict.update(
            {
                "content": content,
                "filename": filename,
            }
        )
        if repository is not UNSET:
            field_dict["repository"] = repository
        if branch is not UNSET:
            field_dict["branch"] = branch
        if scanner is not UNSET:
            field_dict["scanner"] = scanner

        return field_dict

    @classmethod
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        d = dict(src_dict)
        content = d.pop("content")

        filename = d.pop("filename")

        repository = d.pop("repository", UNSET)

        branch = d.pop("branch", UNSET)

        def _parse_scanner(data: object) -> None | str | Unset:
            if data is None:
                return data
            if isinstance(data, Unset):
                return data
            return cast(None | str | Unset, data)

        scanner = _parse_scanner(d.pop("scanner", UNSET))

        secrets_scan_content_request = cls(
            content=content,
            filename=filename,
            repository=repository,
            branch=branch,
            scanner=scanner,
        )

        secrets_scan_content_request.additional_properties = d
        return secrets_scan_content_request

    @property
    def additional_keys(self) -> list[str]:
        return list(self.additional_properties.keys())

    def __getitem__(self, key: str) -> Any:
        return self.additional_properties[key]

    def __setitem__(self, key: str, value: Any) -> None:
        self.additional_properties[key] = value

    def __delitem__(self, key: str) -> None:
        del self.additional_properties[key]

    def __contains__(self, key: str) -> bool:
        return key in self.additional_properties
