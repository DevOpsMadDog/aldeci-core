...
def create_app() -> FastAPI:
    ...
    # -----------------------------------------------------------------------
    # AUTO-MOUNT: discover and mount every *_router.py in this package that
    # was not already explicitly included above.
    #
    # Sleek + scalable: dropping a new *_router.py file into suite-api/apps/api/
    # is all that is needed — no code change here, no registry file.
    #
    # Safety contract:
    #   • _AUTO_SKIP is the exact set of module names already imported explicitly
    #     above — built from the grep of `from apps.api.*` import lines. It is
    #     a frozenset literal so it is O(1) to look up and never changes shape.
    #   • Each import is wrapped in try/except — one bad router never blocks others.
    #   • "app" itself is always in _AUTO_SKIP (would cause circular import).
    # -----------------------------------------------------------------------
    ...
    _auto_pkg_path = [str(_Path(__file__).parent)]
    _auto_pkg_name = "apps.api"

    # Exact set of module names already explicitly imported above.
    # Generated by: grep "from apps.api\." app.py | grep -oP 'from apps\.api\.\K\w+' | sort -u
    _AUTO_SKIP: frozenset[str] = frozenset({
        "app",  # this file itself — never auto-import
        'access_matrix_router', 'admin_connectors_router', 'admin_db_router', 'admin_wizard_router',
        'agentless_snapshot_scan_router', 'ai_orchestrator_router', 'analytics_dashboard_router', 'analytics_engine_router',
        'analytics_routes', 'anomaly_ml_router', 'anomaly_router', 'api_analytics_router',
        'api_docs_router', 'api_gateway_router', 'api_versioning_middleware', 'apikey_router',
        'app_config_router', 'asset_group_router', 'asset_inventory_router', 'attack_surface_manager_router',
        'attack_surface_monitor_router', 'audit_evidence_export_router', 'audit_management_router', 'audit_router',
        'auto_pentest_router', 'auto_waiver_router', 'aws_security_hub_router', 'azure_defender_router',
        'backup_router', 'backup_validator_router', 'behavioral_analytics_router', 'billing_router',
        'breach_response_router', 'bulk_operations_router', 'change_tracker_router', 'changelog_router',
        'cicd_router', 'ciem_router', 'cloud_access_security_router', 'cloud_discovery_router',
        'cloud_identity_router', 'cnapp_router', 'code_to_runtime_router', 'compliance_automation_router',
        'compliance_calendar_router', 'compliance_planner_router', 'compliance_reports_router', 'compliance_seed_router',
        'composite_risk_router', 'config_benchmark_router', 'connector_routes', 'connectors_router',
        'container_scanner_router', 'container_security_connector_router', 'context_engine_router', 'council_enhanced_router',
        'cspm_connector_router', 'cspm_deep_router', 'cspm_engine_router', 'cve_enrichment_router',
        'cwpp_router', 'dashboard_alias_router', 'dashboard_builder_router', 'dast_pentest_router',
        'dast_router', 'data_discovery_router', 'data_exfiltration_router', 'db_security_router',
        'dbir_router', 'design_doc_router', 'detailed_logging', 'developer_portal_router',
        'developer_profiles_router', 'drift_router', 'duckdb_analytics_router', 'epss_router',
        'error_audit_router', 'event_bus_router', 'evidence_chain_router', 'evidence_collector_router',
        'exception_policy_router', 'exec_security_reports_router', 'executive_dashboard_router', 'executive_report_router',
        'fail_router', 'feature_flags_router', 'feed_manager_router', 'feed_registry_router',
        'findings_routes', 'fips_compliance_router', 'fix_engine_router', 'gap_router',
        'ghsa_router', 'github_issues_router', 'graph_rag_router', 'graphrag_router',
        'greynoise_router', 'health', 'hooks_router', 'iam_sso_router',
        'import_router', 'incident_response_router', 'incident_timeline_router', 'ingestion',
        'integration_health_router', 'integration_hub_router', 'integration_marketplace_router', 'intelligent_security_router',
        'inventory_router', 'iot_security_router', 'ip_reputation_router', 'llm_council_router',
        'llm_loop_metrics_router', 'marketplace_router', 'material_change_router', 'mcp_gateway_router',
        'mcp_router', 'mcp_routes', 'metrics_aggregator_router', 'metrics_timeseries_router',
        'mitre_attack_coverage_router', 'mitre_coverage_router', 'ml_vuln_prioritizer_router', 'mlops_supply_chain_router',
        'multi_csp_router', 'n8n_mgmt_router', 'ndr_router', 'network_analyzer_router',
        'network_security_router', 'network_topology_router', 'notification_router', 'nuclei_router',
        'onboarding_router', 'org_export_router', 'org_router', 'org_tier_rate_limit_middleware',
        'outbound_webhooks_router', 'password_policy_router', 'patch_manager_router', 'pentest_router',
        'personas_router', 'phishtank_router', 'pipeline_bom_router', 'pipeline_routes',
        'playbook_routes', 'policies_router', 'policy_engine_router', 'policy_generator_router',
        'posture_advisor_router', 'posture_benchmark_router', 'posture_router', 'pr_gate_router',
        'pr_generator_router', 'prioritizer_router', 'privilege_escalation_detector_router', 'purple_team_router',
        'questionnaire_router', 'rasp_router', 'rate_limit_middleware', 'rate_limit_router',
        'regulatory_tracker_engine_router', 'reports_router', 'retention_router', 'risk_acceptance_router',
        'risk_quantifier_router', 'risk_register_router', 'risk_scoring_router', 'runtime_protection_router',
        'sbom_router', 'scanner_ingest_router', 'scim_router', 'secret_scanner_engine_router',
        'secret_scanner_router', 'secrets_manager_router', 'security_findings_router', 'security_kb_router',
        'security_metrics_router', 'security_query_language_router', 'security_registry_router', 'security_roi_router',
        'security_scorecard_engine_router', 'security_scorecard_router', 'security_telemetry_router', 'self_scan_router',
        'semantic_analyzer_router', 'semgrep_router', 'servicenow_sync_router', 'sigmahq_router',
        'sla_escalation_router', 'slack_bot_router', 'slsa_provenance_router', 'snyk_oss_router',
        'snyk_router', 'soc_automation_router', 'software_license_security_router', 'spamhaus_router',
        'sso_router', 'stream_router', 'stripe_webhook_router', 'sub_apps',
        'supply_chain_intel_router', 'supply_chain_router', 'support_router', 'system_health_router',
        'tag_router', 'tenant_rate_limiter_router', 'tenant_router', 'tfsec_router',
        'third_party_vendor_router', 'threat_actor_router', 'threat_brief_router', 'threat_hunting_router',
        'threat_intel_connector_router', 'threat_intel_sharing_router', 'threat_landscape_router', 'threat_model_generator_router',
        'tor_exit_nodes_router', 'tour_router', 'triage_router', 'trivy_router',
        'trustgraph_maintenance_router', 'trustgraph_quality_router', 'trustgraph_routes', 'unified_dashboard_router',
        'upgrade_path_router', 'urlhaus_router', 'user_analytics_router', 'validation_router',
        'vendor_scorecard_router', 'verification_router', 'version_router', 'versioning_router',
        'vuln_lifecycle_router', 'vuln_risk_router', 'vuln_trend_router', 'vulnerability_correlation_router',
        'waf_router', 'wave_a_code_intel_router', 'wave_d_integrations_router', 'webhook_dlq_router',
        'webhook_events_router', 'webhook_notifications_router', 'webhook_subscriptions_router', 'webhook_verifier_router',
        'websocket_alerts_router', 'workflow_engine_router', 'workflow_router', 'ws_events_router',
        'ws_trustgraph_events_router', 'zero_day_intelligence_router', 'zero_trust_router',
    })

    _auto_mount_count = 0
    _auto_skip_log: list[tuple[str, str]] = []
    _auto_t0 = _time.monotonic()

    for _modinfo in _pkgutil.iter_modules(_auto_pkg_path):
        _mname = _modinfo.name
        if not _mname.endswith("_router"):
            continue
        if _mname in _AUTO_SKIP:
            continue
        try:
            _mod = _importlib.import_module(f"{_auto_pkg_name}.{_mname}")
            _r = getattr(_mod, "router", None)
            if _r is None:
                _auto_skip_log.append((_mname, "no `router` attribute"))
                continue
            app.include_router(_r)
            _auto_mount_count += 1
        except Exception as _exc:  # noqa: BLE001
            _auto_skip_log.append((_mname, f"{type(_exc).__name__}: {str(_exc)[:200]}"))

    _auto_elapsed = _time.monotonic() - _auto_t0
    _logger.info(
        "auto_mount: mounted=%d skipped=%d elapsed=%.1fs",
        _auto_mount_count,
        len(_auto_skip_log),
        _auto_elapsed,
    )
    for _sname, _sreason in _auto_skip_log:
        _logger.warning("auto_mount_skipped: %s — %s", _sname, _sreason)
