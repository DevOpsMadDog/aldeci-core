/**
 * Attack Path Analysis - Live API
 * API: GET /api/v1/attack-paths/paths
 */
  ...
  const load = async () => {
    ...
    try {
      const [itemsRes, statsRes] = await Promise.allSettled([
        apiFetch<any>("/api/v1/attack-paths/paths"),
        apiFetch<any>("/api/v1/attack-paths/stats"),
      ]);
      if (itemsRes.status === "fulfilled") {
        const v = itemsRes.value as any;
        setPaths(Array.isArray(v) ? v : (v.paths ?? v.items ?? v.data ?? []));
      }
  ...
  return (
    <div className="min-h-screen bg-[#0f172a] text-gray-100 p-6 space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-2xl font-bold text-white flex items-center gap-2">
            <Route className="w-6 h-6 text-indigo-400" /> Attack Path Analysis
          </h1>
          <p className="text-gray-400 mt-1">Live data — /api/v1/attack-paths</p>
        </div>
      ...
      {loading ? (
        ...
        <EmptyState icon={Route} title="No paths found" description="Data will appear here once the backend has records." />
      ) : (
        <>
          {stats && (
          ...
          <div className="bg-gray-800 rounded-lg overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-700">
              <h2 className="text-lg font-semibold text-white">Attack Path Analysis ({paths.length})</h2>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead>
                  <tr className="border-b border-gray-700">
                    {Object.keys(paths[0] || {}).slice(0, 6).map(col => (
